...
#' @param numSamp Number of cells to select (if <1 no downsampling occurs)
#' @param k The number of factors
#' @param numRep The number of times to run NMF internally before making the consensus
#' @param rho Internal parameter for cNMF
#' @param t The distance used for filtering out outliers
#' @param getSeruat If true returns Seurat object with cNMF added, else returns results as list
#' @return out list of results (only returned if getSeurat=F)
#' @return seur Seurat object with cNMF added (only returned if getSeurat=T)
#' @export
